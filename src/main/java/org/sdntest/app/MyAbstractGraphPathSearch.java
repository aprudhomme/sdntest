 ...
 * @param <E> edge type
 */
public abstract class MyAbstractGraphPathSearch<V extends Vertex, E extends Edge<V>>
    ...
    protected class DefaultResult implements Result<V, E> {
         ...
         */
        boolean relaxEdge(E edge, double cost, EdgeWeight<V, E> ew, double thresh,
            ...
            double newCost = Math.min(cost, hopCost);
            newCost = Math.min(newCost, thresh);
            boolean relaxed = newCost > oldCost;
            boolean same = Math.abs(newCost - oldCost) <= samenessThreshold;
            if (same || relaxed) {
                //log.info("update vertex: {}, e: {}, cost: {}, same: {}", v, edge, newCost, same);
    ...
    private void buildAllPaths(DefaultResult result, V src, V dst, int maxPaths) {
        //log.info("Starting path construction.");
        ...
        //log.info("Done makeing paths.");
    }

    private boolean containsVertex(E edge, DefaultMutablePath<V, E> path) {
        for (E pathEdge : path.edges()) {
            if (pathEdge.src().equals(edge.src()) || pathEdge.dst().equals(edge.src())) {
                //log.info("Throwing out new path with edge: {}", edge);
                return true;
            }
        }
        return false;
